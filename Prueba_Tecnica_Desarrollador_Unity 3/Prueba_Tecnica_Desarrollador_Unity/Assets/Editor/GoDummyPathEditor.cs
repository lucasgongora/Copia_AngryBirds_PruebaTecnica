...
public class GoDummyPathEditor : Editor
{
		...
		if( _target.nodes.Count > 1 )
		{
			...
			for( var i = 0; i < _target.nodes.Count; i++ )
			{
				...
				else
				{
					...
					var fmh_309_30_638164536126715440 = Quaternion.identity; _target.nodes[i] = Handles.FreeMoveHandle( _target.nodes[i],
					                        handleSize,
					                        new Vector3( 5, 0, 5 ),
				...
				if( Event.current.control && _target.nodes.Count > 3 )
				{
					...
					else if( distanceToNearestNode <= _snapDistance * 2 )
					{
						...
						color.a = 0.3f;
						Handles.color = color;
						Handles.SphereHandleCap( 0, _target.nodes[i], Quaternion.identity, _snapDistance * 2, EventType.MouseDown);
	...
	private void drawArrowBetweenPoints( Vector3 point1, Vector3 point2 )
	{
		...
		Handles.ArrowHandleCap( 0, dir, quat, 25, EventType.ContextClick);
